...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageGroup(_("SSL options: (see the VeriCoin Wiki for SSL setup instructions)"));
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("VeriCoin Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  bitcoin-cli [options] <command> [params]  " + _("Send command to VeriCoin Core") + "\n" +
