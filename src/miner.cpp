...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake)
{
...

    {
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (tx.IsCoinBase() || tx.IsCoinStake() || !IsFinalTx(tx, nHeight, nLockTimeCutoff) || pblock->GetBlockTime() < (int64_t)tx.nTime)
                continue;
...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey, const uint256& hash)
{
    LogPrintf("%s\n", pblock->ToString());
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VeriCoinMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL, hash))
        return error("VeriCoinMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static VeriCoinMiner(CWallet *pwallet)
{
    LogPrintf("VeriCoinMiner started\n");
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in VeriCoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running VeriCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                while(true)
                {
                    ...
                    if (UintToArith256(thash) <= hashTarget)
                    {
                        ...
                        LogPrintf("VeriCoinMiner:\n");
                        LogPrintf("proof-of-work found  \n  powhash: %s  \ntarget: %s\n", thash.GetHex(), hashTarget.GetHex());
                        ProcessBlockFound(pblock, *pwallet, reservekey, thash);
                        SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        LogPrintf("VeriCoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("VeriCoinMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateVeriCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VeriCoinMiner, pwallet));
}
